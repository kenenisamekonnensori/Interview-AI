  ...
  return (
    <main className="min-h-[calc(100vh-5rem)] bg-[radial-gradient(circle_at_50%_0%,oklch(0.3_0.08_275_/_20%),transparent_35rem)] px-5 py-7 sm:px-8 lg:px-10">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2"><span className="flex items-center gap-1.5 rounded-full bg-rose-400/10 px-3 py-1.5 text-xs font-medium text-rose-300"><Radio className="size-3" /> Recording</span><span className="grid size-8 place-items-center rounded-lg bg-white/[.05] text-emerald-300"><Wifi className="size-3.5" /></span></div>
      </div>
      ...
      <div className="overflow-hidden rounded-3xl border border-white/[.08] bg-card/55 p-5 shadow-2xl shadow-black/20 sm:p-7">
        <div className="flex items-center justify-between border-b border-white/[.07] pb-5"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-2xl bg-gradient-to-br from-violet-300 to-indigo-500 text-slate-950"><Radio className="size-4" /></span><div><p className="text-sm font-medium">Your AI interviewer</p><p className="mt-0.5 text-xs text-muted-foreground">Listening for a thoughtful response</p></div></div><span className="text-xs text-muted-foreground">Question 1 of {interview.plan.topics.length}</span></div>
      {completing ? (
      ...
      <aside className="space-y-4"><div className="rounded-2xl border border-white/[.08] bg-card/45 p-5"><p className="text-xs font-medium uppercase tracking-[.13em] text-muted-foreground">Time remaining</p><p className="mt-2 text-4xl font-semibold tracking-[-.05em]">{formatDuration(remaining)}</p><div className="mt-5 h-1.5 overflow-hidden rounded-full bg-white/[.08]"><div className="h-full rounded-full bg-primary" style={{width: `${Math.max(3, remaining / (interview.durationMinutes * 60) * 100)}%`}} /></div></div><div className="rounded-2xl border border-white/[.08] bg-card/45 p-5"><p className="flex items-center gap-2 text-sm font-medium"><ShieldCheck className="size-4 text-emerald-300" /> Session signals</p><div className="mt-4 space-y-3 text-xs text-muted-foreground"><p className="flex justify-between"><span>Microphone</span><span className="text-emerald-300">Ready</span></p><p className="flex justify-between"><span>Network</span><span className="text-emerald-300">Strong</span></p><p className="flex justify-between"><span>Mode</span><span className="text-foreground">Practice</span></p></div></div><TimeCard label="Elapsed" seconds={elapsed} /></aside>
      </div></div>
    </main>
  );
}
