...
const socials = [
  ...
  {
    label: "YouTube",
    href: "https://youtube.com/@interviewai",
...
export function SiteFooter() {
  ...
  return (
    <footer className="relative border-t border-white/[.06]">
      {/* Hairline accent matching the landing design language */}
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 h-64 bg-[radial-gradient(ellipse_at_top,rgba(99,102,241,0.07)_0%,transparent_70%)]" />
      ...
      <div className="relative mx-auto max-w-5xl px-6">
        ...
        <div className="flex flex-col items-start justify-between gap-6 pt-16 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="mt-1 text-sm text-white/50">
              Start your first AI mock interview today — free, no credit card required.
            </p>
          </div>
          <Link
            className="btn-premium inline-flex h-10 shrink-0 items-center gap-2 rounded-xl px-5 text-sm font-semibold text-white"
            href="/sign-up"
          >
        ...
        <div className="mt-14 grid gap-12 sm:grid-cols-2 lg:grid-cols-[1.6fr_1fr_1fr_1fr]">
          <div>
            ...
            <Link
              className="mt-6 inline-flex items-center gap-2 rounded-full border border-emerald-400/15 bg-emerald-400/5 px-3 py-1 text-xs font-medium text-emerald-300/80 transition-colors hover:text-emerald-300"
              href="/status"
            >
        ...
        {/* Bottom bar */}
        <div className="mt-14 flex flex-col items-center justify-between gap-4 border-t border-white/[.05] py-8 text-xs text-white/35 sm:flex-row">
          <p>© {year} InterviewAi. All rights reserved.</p>
          <ul className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2">
            {legalLinks.map((link) => (
              <li key={link.label}>
                <Link className="transition-colors hover:text-white/70" href={link.href}>
                  {link.label}
                </Link>
              </li>
            ))}
